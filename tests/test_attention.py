...
class TestStreamAttention:
    ...
    def test_ring_attention_correctness(self, config, device):
        ...
        # Compare
        torch.testing.assert_close(
            ring_output, ref_output,
            rtol=1e-1, atol=1e-1,
            msg="Ring Attention mismatch"
        )
    ...
    def test_star_attention_correctness(self, config, device):
        ...
        # Create context
        context = torch.randn(
            batch_size, context_len, config.num_heads, config.head_dim,
        ...
        # Create query
        query = torch.randn(
            batch_size, query_len, config.num_heads, config.head_dim,
            device=device, dtype=torch.float16
        )
    ...
    def test_memory_efficiency(self, config, device):
        ...
        for i, seq_len in enumerate(seq_lengths):
            if seq_len >= 4096:
                assert memory_usage['star'][i] < memory_usage['flash'][i] * 1.5, \
    ...
    def test_kv_cache_compression(self, device):
        ...
        for method in ['importance', 'chunk', 'quantized', 'hybrid']:
            ...
            # Compress
            comp_k, comp_v, stats = compressor.compress(keys, values, compression_ratio=4.0)
    ...
    def test_performance_scaling(self, config, device, seq_len):
        ...
        assert elapsed_time < seq_len / 100, f"Performance too slow for seq_len={seq_len}"
