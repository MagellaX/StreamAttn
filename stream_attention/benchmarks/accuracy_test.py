...
def main():
	parser = argparse.ArgumentParser(description="Accuracy comparison: FusedOnlineAttention vs FlashAttentionV3")
	...
	v = torch.randn(args.batch, args.seq, args.heads, args.dim, device=device, dtype=dtype)

	with torch.no_grad():
		o1 = fused(q, k, v, causal=True)
		o2 = fa3(q, k, v, causal=True)
		diff = (o1.float() - o2.float()).abs()
		print(f"Mean abs diff: {diff.mean().item():.6f}, Max abs diff: {diff.max().item():.6f}")
