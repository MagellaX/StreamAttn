...
def gate1_inline_projection_splitk_attention_triton_forward(
    ...
    *,
    ...
    chunk_max = torch.empty(batch, heads, num_chunks, device=query.device, dtype=torch.float32)
    chunk_den = torch.empty(batch, heads, num_chunks, device=query.device, dtype=torch.float32)
    chunk_num = torch.empty(batch, heads, num_chunks, dim, device=query.device, dtype=torch.float32)
    raw_stats = (
