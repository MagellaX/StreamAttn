...
def replace_llama_attention(model, config: StreamAttentionConfig, module_name: str = "self_attn") -> int:
	"""
...
def replace_attention_generic(model, config: StreamAttentionConfig, name_pattern: str = "attn") -> int:
...
class GPT2AttentionAdapter(nn.Module):
	...
	def forward(
		...
		output = self.inner(hidden_states=hidden_states, past_key_value=past_kv, use_cache=use_cache, causal=True)
		if use_cache:
			attn_out, kv = output
...
def replace_gpt2_attention(model, config: StreamAttentionConfig, module_name: str = "attn") -> int:
