...
class ImportanceBasedCompressor(KVCacheCompressor):
    ...
    def compress(
        ...
        memory_saved = (original_size - compressed_size) * keys.element_size() / (1024 * 1024)
    ...
    def compress(
        ...
        for chunk_idx in range(num_chunks):
            ...
            if chunk_len <= keep_per_chunk:
                ...
                indices = indices.unsqueeze(-1).unsqueeze(-1).expand(
                    -1, -1, num_heads, head_dim
                )
...
class QuantizedCompressor(KVCacheCompressor):
    ...
    def compress(
        ...
        memory_saved = (original_size - compressed_size) * 2 / (1024 * 1024)  # 2 bytes per FP16
...
class MemoryEfficientAttention(nn.Module):
    ...
    def _attention_forward(
        ...
        if attention_mask is not None:
            ...
            scores = scores.masked_fill(~chunk_mask.unsqueeze(1).unsqueeze(-1), float('-inf'))
