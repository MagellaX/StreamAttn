    ...
    def fused_online_attention_kernel(
        ...
        for start_n in range(0, N, TILE_N):
            ...
            if HAS_MASK:
                ...
                mask_vals = tl.load(mask_ptrs, mask=mask_mask, other=0.0).to(tl.float32)
                masked = mask_vals < 0.0
                qk = tl.where(masked, float("-inf"), qk)
            ...
            if HAS_MASK:
                exp_qk = tl.where(masked, 0.0, exp_qk)
    ...
    def fused_online_attention_bwd_kernel(
        ...
        for start_n in range(0, N, TILE_N):
            ...
            if HAS_MASK:
                ...
                mask_vals = tl.load(mask_ptrs, mask=mask_load_mask, other=0.0).to(tl.float32)
                masked = mask_vals < 0.0
                logits = tl.where(masked, float("-inf"), logits)
