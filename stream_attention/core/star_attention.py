...
class StarAttention(nn.Module):
    ...
    def _create_context_blocks(
        ...
        anchor_block = blocks[0][:, :self.anchor_block_size] if include_anchor else None
    ...
    def _distribute_blocks_to_hosts(
        ...
        anchor_block: Optional[torch.Tensor]
    ) -> List[torch.Tensor]:
    ...
    def phase2_query_processing(
        ...
        for block_id, (key_cache, value_cache) in state.kv_caches.items():
            ...
            scores = torch.matmul(
                query.transpose(1, 2),
                key_cache.transpose(1, 2).transpose(-2, -1)
            ) / math.sqrt(head_dim)
            ...
            local_stats.append(AttentionStats(
        ...
        if self.distributed:
            ...
            for local_output, stats in zip(local_outputs, local_stats):
                ...
                if self.is_query_host:
                    ...
                    gathered_max_scores = [torch.zeros_like(stats.max_scores) for _ in range(self.world_size)]
                    gathered_sum_exps = [torch.zeros_like(stats.sum_exp) for _ in range(self.world_size)]
                else:
            ...
            output = self._aggregate_local_attention(
                local_outputs, local_stats
            )
        
        state.phase2_time = time.time() - start_time
        
        # Calculate speedup
        baseline_time = (batch_size * query_len * num_heads * head_dim * 
                        state.kv_caches[0][0].shape[1] * len(state.kv_caches)) / 1e9
    ...
    def _aggregate_local_attention(
        ...
        local_stats: List[AttentionStats]
    ) -> torch.Tensor:
    ...
    def _compress_kv_cache(
        ...
        indices = indices.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, num_heads, head_dim)
    ...
    def process_query(
        self,
        query: torch.Tensor,
        state: Optional[StarAttentionState] = None
    ) -> torch.Tensor:
        """
