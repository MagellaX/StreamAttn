...
class RingAttention(nn.Module):
    ...
    def __init__(
        ...
        if dist.is_initialized():
            ...
            self.world_size = 1
            self.rank = 0
            logger.warning("Distributed not initialized, Ring Attention running in single-device mode")
    ...
    def _compute_block_attention(
        ...
        scores = torch.matmul(
            query_block.transpose(1, 2),
            key_block.transpose(1, 2).transpose(-2, -1)
        ) / math.sqrt(head_dim)
        
        # Apply causal mask if needed
        if causal_mask:
            # Calculate global positions
            q_pos = state.query_block_idx * self.block_size + torch.arange(q_len, device=query_block.device)
            kv_pos = state.key_value_block_idx * self.block_size + torch.arange(kv_len, device=key_block.device)
    ...
    def _combine_block_outputs(
        ...
        combined_lse = max_lse + torch.log(exp_diff1.squeeze(-1) + exp_diff2.squeeze(-1))
