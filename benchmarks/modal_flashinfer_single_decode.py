"""Modal runner for FlashInfer single-decode vs StreamAttn decode profiling."""
...
app = modal.App("streamattn-flashinfer-single-decode")
...
def _profile(
    *,
    ...
    if cost_path:
        cmd.extend(["--decode-cost-json", cost_path])
    ...
    payload["flashinfer_config"] = flashinfer_config
    return payload
...
def main(
    ...
    decode_cost_payload = (
        Path(decode_cost_json).read_text(encoding="utf-8")
        if decode_cost_json
        else ""
    )
    kwargs = {
    ...
    if target == "h100":
        ...
        raise ValueError("target must be h100 or a100")
