"""Modal runner for KV-group hybrid component-floor diagnostics."""
...
app = modal.App("streamattn-gate0-hybrid-component-floors")
...
def _json_from_cmd(cmd: list[str], *, env: dict[str, str]) -> dict[str, Any]:
    print(f"[modal-component-floors] running: {' '.join(cmd[:5])} ...", flush=True)
...
def _run(
    *,
    ...
    repeats: int,
    tk_num_chunks: int,
    strong_margin_ms: float,
):
    env = os.environ.copy()
    env["PYTHONUNBUFFERED"] = "1"
    env["PYTHONPATH"] = "/root/StreamAttn" + os.pathsep + env.get("PYTHONPATH", "")

    prompt_file = "/tmp/streamattn_component_floors_prompt.txt"
    capture_dir = "/tmp/streamattn_component_floors_qkv"
    ...
    for row in rows:
        ...
        cmd = [
            "python",
            "-u",
            "/root/StreamAttn/benchmarks/profile_gate0_hybrid_component_floors.py",
            "--q-path",
            row["q_path"],
            "--k-path",
            row["k_path"],
...
def main(
    ...
    prompt_kind: str = "",
    prompt_file: str = "",
    prompt_type: str = "needle_component_floors_l8_32k",
    ...
    result = profile_h100.remote(
        ...
        strong_margin_ms=strong_margin_ms,
    )
