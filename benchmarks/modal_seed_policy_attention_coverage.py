"""Modal runner for prompt-aware attention coverage diagnostics."""
...
app = modal.App("streamattn-seed-policy-attention-coverage")
...
def _run_cmd(cmd: list[str], *, env: dict[str, str]) -> None:
    print(f"[modal-attention-coverage] running: {' '.join(cmd[:5])} ...", flush=True)
...
def _run(**kwargs) -> dict[str, Any]:
    ...
    if failure_artifact_json:
        remote_failure_artifact = "/root/StreamAttn/artifacts/gate0/attention_coverage_failure_source.json"
    ...
    cmd = [
        "python",
        "-u",
        "/root/StreamAttn/benchmarks/profile_seed_policy_attention_coverage.py",
        "--model",
        kwargs["model"],
        "--prompt-file",
        kwargs["prompt_file"],
...
def main(
    ...
    if failure_artifact:
        ...
        if path.exists():
            ...
            print(f"[modal-attention-coverage] warning: failure artifact not found: {failure_artifact}", flush=True)
    ...
    result = profile_h100.remote(
        ...
        max_rows=max_rows,
        remote_output_json="/root/StreamAttn/artifacts/gate0/attention_coverage/l24_l26_l27.json",
    )
    ...
    if output_json:
        ...
        compact = {
            ...
            "row_steps": result.get("row_steps"),
