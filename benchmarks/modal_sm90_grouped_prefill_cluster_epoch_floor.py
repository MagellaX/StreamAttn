"""Bounded Modal H100 runner for the cluster multicast attention epoch."""
...
image = (
    ...
    .add_local_file(
        "benchmarks/profile_sm90_grouped_prefill_cluster_epoch_floor.py",
        remote_path=(
            "/root/StreamAttn/benchmarks/"
            "profile_sm90_grouped_prefill_cluster_epoch_floor.py"
        ),
        copy=True,
    )
    .add_local_dir(
...
app = modal.App("streamattn-sm90-grouped-prefill-cluster-epoch-floor")
...
def run(
    *,
    ...
    command = [
        ...
        "--build-dir",
        "/tmp/streamattn-prefill-cluster-epoch-floor-build",
        "--output-json",
        "/tmp/result.json",
        "--verbose-build",
    ]
...
def main(
    ...
    output_json: str = (
        "artifacts/gate0/"
        "sm90_grouped_prefill_cluster_epoch_floor_modal_h100_20260902.json"
    ),
) -> None:
    payload = run.remote(
