"""Modal runner for synthetic seed-only KV scaling."""
...
app = modal.App("streamattn-seed-only-synthetic-scaling")
...
def _json_from_cmd(cmd: list[str], *, env: dict[str, str], tail: int = 5000) -> dict[str, Any]:
    print(f"[seed-scaling] running: {' '.join(cmd[:5])} ...", flush=True)
...
def _run(**kwargs) -> dict[str, Any]:
    ...
    cmd = [
        "python",
        "/root/StreamAttn/benchmarks/profile_seed_only_synthetic_scaling.py",
        "--dtype",
        kwargs["dtype"],
        "--q-heads",
        str(kwargs["q_heads"]),
...
def main(
    ...
    kv_heads: int = 2,
    dim: int = 64,
    kv_lens: str = "4096,8192,16384,32768,65536,131072",
    block_size: int = 32,
    sink_blocks: int = 2,
    recent_blocks: int = 2,
