"""Run the SM90 D128 TMA floor on Lightning and always delete the job."""
...
BASE_SHA = "de6b3f1"
RESULT_SCHEMA = "streamattn.sm90_d128_tma_pipeline_floor.v1"
...
def _overlay_b64() -> str:
    ...
    with tarfile.open(fileobj=buffer, mode="w:gz", compresslevel=9) as archive:
        for relative in (
            ...
            "stream_attention/backends/sm90/tma_pipeline_floor.py",
            "stream_attention/backends/sm90/tma_pipeline_floor_sources.py",
        ):
...
def _job_command(args: argparse.Namespace) -> str:
    ...
    return "\n".join(
        [
            ...
            "cd /root/StreamAttn",
            "python -u benchmarks/profile_sm90_d128_tma_pipeline_floor.py "
            f"--total-tiles {args.total_tiles} --tiles-per-cta {args.tiles_per_cta} "
            f"--warmup {args.warmup} --iters {args.iters} --repeats {args.repeats} "
            "--cutlass-root /tmp/flashmla-etap/csrc/cutlass "
            "--build-dir /tmp/streamattn-tma-floor-build "
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    try:
        ...
        if result is None:
            raise RuntimeError("could not parse TMA floor result")
...
def main() -> None:
    ...
    parser.add_argument("--name", default="streamattn-sm90-d128-tma-floor")
    parser.add_argument(
        "--teamspace-id",
