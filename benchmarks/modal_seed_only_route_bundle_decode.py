...
def _run(**kwargs) -> dict[str, Any]:
    ...
    cmd = [
        ...
        kwargs["policy_names"],
        "--product-fast-path",
        kwargs["product_fast_path"],
        "--bucket-route-policy",
        kwargs["bucket_route_policy"],
        "--route-risk-tier",
        kwargs.get("route_risk_tier", "bucket_only"),
        "--prompt-kinds",
        kwargs["prompt_kinds"],
        "--prompt-repeat",
    ...
    if kwargs["seed_attention_backend"] != "triton":
        cmd.extend(["--seed-attention-backend", kwargs["seed_attention_backend"]])
    if kwargs["exact_refresh_interval"] > 0:
...
def main(
    ...
    product_fast_path: str = "none",
    bucket_route_policy: str = "none",
    product_strict: bool = True,
    route_risk_tier: str = "bucket_only",
    ...
    result = profile_h100.remote(
        ...
        product_fast_path=product_fast_path,
