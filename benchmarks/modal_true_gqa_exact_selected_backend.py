"""Modal runner for exact selected-head true-GQA backend profiling."""
...
app = modal.App("streamattn-true-gqa-exact-selected-backend")
...
def _json_from_cmd(cmd: list[str], *, env: dict[str, str]) -> dict[str, Any]:
    print(f"[modal-exact-selected] running: {' '.join(cmd[:5])} ...", flush=True)
...
def _run(
    *,
    ...
    measure_triton_splitk: bool,
):
    env = os.environ.copy()
    env["PYTHONPATH"] = "/root/StreamAttn" + os.pathsep + env.get("PYTHONPATH", "")

    prompt_file = "/tmp/streamattn_exact_selected_prompt.txt"
    capture_dir = "/tmp/streamattn_exact_selected_qkv"
    ...
    for row in rows:
        ...
        profile_cmd = [
            "python",
            "/root/StreamAttn/benchmarks/profile_true_gqa_exact_selected_backend.py",
            "--q-path",
            row["q_path"],
            "--k-path",
            row["k_path"],
        ...
        if measure_triton_splitk:
            profile_cmd.append("--measure-triton-splitk")
...
def main(
    ...
    prompt: str = "needle retrieval sparse attention seed only online softmax gate0 decode long context",
    prompt_file: str = "",
    prompt_type: str = "needle_exact_selected_l8_32k",
    ...
    group_warmup: int = 3,
    group_iters: int = 10,
    measure_flashinfer: bool = True,
    flashinfer_tensor_cores: bool = False,
    measure_triton_splitk: bool = True,
    output_json: str = "",
):
    if prompt_file:
    ...
    result = profile_h100.remote(
        ...
        measure_triton_splitk=measure_triton_splitk,
    )
