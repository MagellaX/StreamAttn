"""Modal runner for the KV-group hybrid backend realism benchmark."""
...
app = modal.App("streamattn-gate0-kv-group-hybrid-backend")
...
def _json_from_cmd(cmd: list[str], *, env: dict[str, str]) -> dict[str, Any]:
    print(f"[modal-kv-hybrid-backend] running: {' '.join(cmd[:5])} ...", flush=True)
...
def _run(
    *,
    ...
    measure_cuda_graph_wall: bool,
    measure_graph_mutation_check: bool,
    ...
    prompt_file = "/tmp/streamattn_kv_hybrid_backend_prompt.txt"
    capture_dir = "/tmp/streamattn_kv_hybrid_backend_qkv"
    ...
    for row in rows:
        ...
        profile_cmd = [
            "python",
            "-u",
            "/root/StreamAttn/benchmarks/profile_gate0_kv_group_hybrid_backend.py",
            "--q-path",
            row["q_path"],
            "--k-path",
            row["k_path"],
        ...
        if measure_cuda_graph_wall:
            profile_cmd.append("--measure-cuda-graph-wall")
        if measure_graph_mutation_check:
            profile_cmd.append("--measure-graph-mutation-check")
...
def main(
    ...
    prompt_kind: str = "",
    prompt_file: str = "",
    prompt_type: str = "needle_kv_group_hybrid_backend_l8_32k",
    ...
    measure_cuda_graph: bool = True,
    measure_cuda_graph_wall: bool = True,
    measure_graph_mutation_check: bool = True,
    ...
    result = profile_h100.remote(
        ...
        measure_cuda_graph_wall=measure_cuda_graph_wall,
        measure_graph_mutation_check=measure_graph_mutation_check,
