"""Modal runner for KV-group seed-only CUDA backend profiling."""
...
app = modal.App("streamattn-seed-only-kv-group-cuda")
...
def _json_from_cmd(cmd: list[str], *, env: dict[str, str], tail: int = 3000) -> dict[str, Any]:
    print(f"[kv-group-cuda] running: {' '.join(cmd[:5])} ...", flush=True)
...
def _run(
    *,
    ...
    prompt_file = "/tmp/streamattn_kv_group_seed_prompt.txt"
    capture_dir = "/tmp/streamattn_kv_group_seed_qkv"
    ...
    for row in rows:
        ...
        profile = _json_from_cmd(
            [
                "python",
                "/root/StreamAttn/benchmarks/profile_seed_only_kv_group_cuda.py",
                "--q-path",
                row["q_path"],
                "--k-path",
                row["k_path"],
    ...
    return {
        "schema": "streamattn.gate0.seed_only_kv_group_cuda_sweep.v1",
        "capture": {
            "model_id": model,
            "prompt_type": prompt_type,
            "layers": layers,
...
def main(
    ...
    prompt_file: str = "",
    prompt_type: str = "needle_seed_only_kv_group_cuda_l8_32k",
