"""Modal runner for true-GQA fused-hybrid Gate-0 profiling."""
...
app = modal.App("streamattn-gate0-true-gqa")
...
def _json_from_cmd(cmd: list[str], *, env: dict[str, str]) -> dict[str, Any]:
    print(f"[modal-true-gqa] running: {' '.join(cmd[:5])} ...", flush=True)
...
def _run(
    *,
    ...
    prompt_file = "/tmp/streamattn_gate0_true_gqa_prompt.txt"
    policy_file = "/tmp/streamattn_gate0_true_gqa_policy.json"
    capture_dir = "/tmp/streamattn_gate0_true_gqa_qkv"
    ...
    for row in rows:
        ...
        print(
            "[modal-true-gqa] profiling "
            f"layer={row.get('layer_id')} true_kv_heads={true_kv_heads} shape={row.get('shape')}",
            flush=True,
        )
        profile_cmd = [
            "python",
            "/root/StreamAttn/benchmarks/profile_gate0_true_gqa.py",
            "--q-path",
            row["q_path"],
            "--k-path",
            row["k_path"],
