"""Run no-sync dynamic selected-paged H100 phase measurements."""
...
app = modal.App("streamattn-paged-dynamic-selected-decode")
...
def profile_h100(kwargs: dict[str, object]) -> dict[str, object]:
    ...
    from benchmarks.profile_paged_dynamic_selected_decode import profile

    batches = [int(value) for value in _items(str(kwargs["batches"]))]
    selected_values = [
        int(value) for value in _items(str(kwargs["selected_tokens"]))
    ]
    route_modes = _items(str(kwargs["route_modes"]))
    base = {
        key: value
        for key, value in kwargs.items()
        if key not in {"batches", "selected_tokens", "route_modes"}
    }
    ...
    for batch in batches:
        for selected_tokens in selected_values:
            for route_mode in route_modes:
                ...
                print(
                    f"[dynamic-selected] B={batch} N={args.kv_len} "
                    f"S={selected_tokens} mode={route_mode}",
                    flush=True,
                )
                row = profile(args)
                cells.append(row)
                print(
                    f"[dynamic-selected] prepare={row['gpu_prepare_ms']:.5f} ms "
                    f"total={row['dynamic_prepare_and_selected_ms']:.5f} ms "
                    f"flashinfer={row['flashinfer_ms']:.5f} ms "
                    f"speedup={row['dynamic_speedup_vs_flashinfer']:.3f}x "
                    f"paired_min={row['paired']['speedup_min']:.3f}x",
                    flush=True,
                )
    return {
        "schema": "streamattn.paged_dynamic_selected_decode_matrix.v1",
        "device": cells[0]["device"] if cells else "unknown",
        "summary": {
            "cells": len(cells),
            "correct_cells": sum(
                float(row["max_abs_error_dynamic"]) <= float(base["atol"])
                for row in cells
            ),
...
def main(
    batches: str = "1,4,8",
    selected_tokens: str = "384,2048",
    route_modes: str = "shared,alternating,disjoint",
    kv_len: int = 32768,
    q_heads: int = 16,
    kv_heads: int = 2,
    head_dim: int = 128,
    ...
    result = profile_h100.remote(
        {
            ...
            "warmup": warmup,
            "repeats": repeats,
            "lowering_repeats": lowering_repeats,
