"""Bounded Modal H100 runner for the D128 warp-specialized cp.async floor."""

from __future__ import annotations

from pathlib import Path

import modal


image = (
    modal.Image.from_registry("pytorch/pytorch:2.7.1-cuda12.8-cudnn9-devel")
    .apt_install("git", "ninja-build")
    .pip_install("ninja")
    .run_commands(
        "git clone --filter=blob:none --no-checkout "
        "https://github.com/pengcuo/FlashMLA-ETAP.git /opt/flashmla-etap && "
        "cd /opt/flashmla-etap && git sparse-checkout init --cone && "
        "git sparse-checkout set csrc/cutlass/include && "
        "git fetch --depth=1 origin 39e616041ae6fb1243a0f6ac891e72d576b640e5 && "
        "git checkout 39e616041ae6fb1243a0f6ac891e72d576b640e5"
    )
    .add_local_file(
        "benchmarks/profile_sm90_d128_ws_cp_async_floor.py",
        remote_path="/root/StreamAttn/benchmarks/profile_sm90_d128_ws_cp_async_floor.py",
        copy=True,
    )
    .add_local_dir(
        "stream_attention",
        remote_path="/root/StreamAttn/stream_attention",
        copy=True,
    )
)

app = modal.App("streamattn-sm90-d128-ws-cp-floor")


@app.function(image=image, gpu="H100", timeout=30 * 60)
def run(
    *, warmup: int, iters: int, repeats: int, splits: str, consumer_registers: str
) -> str:
    import os
    import subprocess

    os.chdir("/root/StreamAttn")
    subprocess.run(
        [
            "python",
            "-u",
            "benchmarks/profile_sm90_d128_ws_cp_async_floor.py",
            "--splits",
            splits,
            "--consumer-registers",
            consumer_registers,
            "--warmup",
            str(warmup),
            "--iters",
            str(iters),
            "--repeats",
            str(repeats),
            "--cutlass-root",
            "/opt/flashmla-etap/csrc/cutlass",
            "--build-dir",
            "/tmp/streamattn-ws-cp-build",
            "--output-json",
            "/tmp/result.json",
            "--verbose-build",
        ],
        check=True,
    )
    return Path("/tmp/result.json").read_text(encoding="utf-8")


@app.local_entrypoint()
def main(
    warmup: int = 10,
    iters: int = 100,
    repeats: int = 5,
    splits: str = "4,8,16",
    consumer_registers: str = "96,112,128,160",
    output_json: str = (
        "artifacts/gate0/sm90_d128_ws_cp_async_floor_modal_h100_20260819.json"
    ),
) -> None:
    payload = run.remote(
        warmup=warmup,
        iters=iters,
        repeats=repeats,
        splits=splits,
        consumer_registers=consumer_registers,
    )
    output = Path(output_json)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(payload, encoding="utf-8")
    print(f"wrote {output}")
