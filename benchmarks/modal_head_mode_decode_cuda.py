"""Modal runner for scheduler-level CUDA head-mode decode prototype."""
...
app = modal.App("streamattn-head-mode-decode-cuda")
...
def _profile_cmd(
    *,
    ...
    cmd = [
        "python",
        "/root/StreamAttn/benchmarks/profile_head_mode_decode_cuda.py",
        "--dtype",
        dtype,
        "--seed-heads",
        seed_heads,
...
def _run_synthetic(
    *,
    ...
    return _json_from_cmd(cmd, env=env, prefix="head-mode-cuda-synth")
...
def _run_real(
    *,
    ...
    prompt_file = "/tmp/streamattn_head_mode_decode_prompt.txt"
    capture_dir = "/tmp/streamattn_head_mode_decode_qkv"
    ...
    for row in rows:
        ...
        profile = _json_from_cmd(cmd, env=env, prefix="head-mode-cuda-profile")
        profile["capture"] = {
            "model_id": model,
...
def main(
    ...
    prompt: str = "needle retrieval sparse attention seed only online softmax head mode backend long context",
    prompt_file: str = "",
    prompt_type: str = "needle_head_mode_cuda_l8_32k",
