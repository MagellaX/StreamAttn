...
## Hybrid Stress Matrix Result

The full 32-step H100 matrix completed. No tested hybrid route passed:

```text
L27 exact + L26 seed:
  speedup: 1.13601x
  KL max:  0.308575
  top1/sample changes: 17 / 12

L27 exact + L26 dynamic extreme4:
  speedup: 0.14305x
  KL max:  0.248408
  top1/sample changes: 14 / 10

L27 exact + L26 dynamic qk_block_max:
  speedup: 0.28390x
  KL max:  0.250073
  top1/sample changes: 15 / 11

L26/L27 exact:
  speedup: 1.10697x
  KL max:  0.222381
  top1/sample changes: 8 / 7

L24/L26/L27 exact:
  speedup: 1.07884x
  KL max:  0.212279
  top1/sample changes: 6 / 9
```

Decision:

```text
no stress hybrid route promotion
stress and unknown risk tiers use exact_native
validated risk tier keeps the product fast path
```

The matrix also showed that coarse bucket names are insufficient. Adversarial
stress-form `code` and `long_doc` require exact under the current gate even
though separately validated `code` and `long_doc` prompts can use the fast
route. The router therefore needs an explicit risk/admissibility tier or a
runtime verifier signal.

Next research:

```text
build stress admissibility dataset
collect validated positive examples with the same features
train/calibrate an interpretable seed/dynamic/exact verifier
```
